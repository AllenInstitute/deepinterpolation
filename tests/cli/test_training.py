...
def test_training_cli(generator_args, training_args, network_args, monkeypatch):
    ...
    model_path = os.path.join(
        args["training_params"]["output_dir"],
        args["run_uid"] + "_" + args["training_params"]["model_string"] + "_model.h5",
    )
...
def test_integration_cli_ephys_inference(tmp_path):
    ...
    generator_test_param["randomize"] = True
    ...
    model_path = os.path.join(
        args["training_params"]["output_dir"],
        args["run_uid"] + "_" + training_param["model_string"] + "_model.h5",
    )
